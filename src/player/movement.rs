...
use crate::chunk::{CurrentChunk, HEIGHT};
use crate::prelude::*;
use bevy::{
...
pub(super) fn player_move(
    ...
    mut query: Query<(&FlyCam, &mut Transform, &Cage)>,
    mut current_chunk: ResMut<CurrentChunk>,
    //    mut query: Query<&mut Transform, With<FlyCam>>,
) {
    if let Ok(window) = primary_window.get_single() {
        for (_camera, mut transform, cage) in query.iter_mut() {
            ...
            if zblock == Block::AIR && xzblock == Block::AIR {
